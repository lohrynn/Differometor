...
class BenchmarkResult:
    ...
    calls_to_success: Float[
        Array, "n_time_steps"
    ]  # Using Float for consistency, cast to int when needed
...
class Benchmark:
    ...
    def _evaluate_runs(
        ...
        for i_time, wall_time in enumerate(self._wall_time_steps):
            ...
            fraction_of_success = fraction_of_success.at[i_time].set(
                n_successes / n_runs
            )
            ...
            successful_iters = jnp.where(
                n_successes > 0,
                jnp.mean(jnp.where(has_success, first_success_iter, 0.0))
                * n_runs
                / jnp.maximum(n_successes, 1),
                float(max_iter),
            )

            time_to_success = time_to_success.at[i_time].set(
                successful_iters * time_per_iteration
            )
            calls_to_success = calls_to_success.at[i_time].set(
                jnp.round(successful_iters).astype(jnp.int32)
            )
            ...
            # TIME PER CALL
            time_per_call = time_per_call.at[i_time].set(
            ...
            auc_top_1 = auc_top_1.at[i_time].set(
            ...
            auc_top_10 = auc_top_10.at[i_time].set(
                jnp.trapezoid(avg_top_losses, dx=time_per_iteration)
            )
            ...
            if n_successes > 1:
                diversity_overall_val, diversity_nn_val = self._calculate_diversity(
                    successful_params, n_successes
                )
            else:
                diversity_overall_val, diversity_nn_val = 0.0, 0.0

            solution_diversity_overall = solution_diversity_overall.at[i_time].set(
                diversity_overall_val
            )
            solution_diversity_nn = solution_diversity_nn.at[i_time].set(
                diversity_nn_val
            )
    ...
    def run_benchmark(self, save_to_file: bool = True) -> list[BenchmarkResult]:
        ...
        for i, config in enumerate(self._configs, 1):
            ...
            print(
                f"  Success rate (final): {float(result.fraction_of_success[-1]):.1%}"
            )
            ...
            print(
                f"  Diversity (overall): {float(result.solution_diversity_overall[-1]):.4f}"
            )
    ...
    def print_summary(self, results: list[BenchmarkResult]):
        ...
        # Header
        print(
            f"{'Algorithm':<20} {'Success%':>10} {'Min Loss':>12} {'Avg Loss':>12} {'Time(s)':>10} {'Diversity':>10}"
        )
        print("-" * 70)
        ...
        for result in results:
            ...
            print(
                f"{name:<20} {success:>9.1f}% {min_loss:>12.6f} {avg_loss:>12.6f} {time:>10.2f} {diversity:>10.4f}"
            )
